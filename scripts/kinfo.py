...
class KubeInfo:
    ...
    def get_node_data_single_input(self,sort_by_cpu=False,include_controller=False):
        ...
        for node in data:
            ...
            single_row_of_data.append(np.round(np.round(node['total_memory_used']/node['memory_capacity'],4),4))
            single_row_of_data.append(np.round(np.round(node['pod_count']/node['pod_limit'],4)))
